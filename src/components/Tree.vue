  ...
  export default {
    ...
    methods: {
      handleDragstart (leafComponent) {
        ...
        const treeLayer = vm.$refs.treeLayer.getStage()
        const stage = vm.$refs.stage.getStage()
        // moving to another layer will improve dragging performance
        shape.moveTo(treeLayer)
        stage.draw()
        document.body.style.cursor = 'pointer'
        leafComponent.config.stroke = '#ffffff'
        leafComponent.config.strokeWidth = 1
        leafComponent.config.scaleX = leafComponent.config.startScale * 2
        leafComponent.config.scaleY = leafComponent.config.startScale * 2
        leafComponent.config.x -= 16
        leafComponent.config.y -= 16
      },
      handleDragend (leafComponent) {
        ...
        shape.to({
          ...
          x: leafComponent.config.x + 16,
          y: leafComponent.config.y + 16,
          shadowOffsetX: 5,
          shadowOffsetY: 5
        })
      },
